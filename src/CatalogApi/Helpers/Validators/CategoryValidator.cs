...
public class CreateCategoryDtoValidator : AbstractValidator<CreateCategoryDto> {
  public CreateCategoryDtoValidator(ICategoryRepository repository) {
    RuleFor(static e => e.Name).NotEmpty().WithMessage(CategoryServiceErrors.InvalidName.Value).CustomAsync(async (name, ctx, CancellationToken) => {
      var nameAlreadyInUse = await repository.NameAlreadyInUse(name);
      if (nameAlreadyInUse) {
        ctx.AddFailure(CategoryServiceErrors.NameAlreadyExists.Value);
      }
    });
  }
}
...
public class UpdateCategoryDtoValidator : AbstractValidator<UpdateCategoryDto> {
  public UpdateCategoryDtoValidator(ICategoryRepository repository, long id) {
    RuleFor(static e => e.Name).NotEmpty().WithMessage(CategoryServiceErrors.InvalidName.Value)
